...
void display(struct node *T,int indent)  {//对抽象语法树的先根遍历
  ...
  case BREAK:         printf("%*c`--break\n",indent,' ');
	...
	case COMP_STM:      printf("%*c`--Block\n",indent,' ');
                        ...
                        // printf("%*c复合语句的语句部分：\n",indent+3,' ');
                        display(T->ptr[1],indent+6);      //显示语句部分y
                        break;
  ...
  case FOR:             printf("%*c`--for\n",indent,' ');
    ...
    case DEF_LIST:      display(T->ptr[0],indent);    //显示该局部变量定义列表中的第一个
                        ...
                        break;
    case VAR_DEF:       printf("%*c|--Decl\n",indent,' ');
                        display(T->ptr[0],indent+3);   //显示变量类型
                        display(T->ptr[1],indent+3);   //显示该定义的全部变量名
                        break;
    case ARR:       printf("%*c|--ArrDecl %s %s",indent,' ',T->type==INT?"int":"float",T->type_id);
                        break;
    case DIM:           int i = 1;
                        while (T) {
                          if(T->type == INT){
    ...
    case DEC_LIST:      printf("%*|--VarDecl\n",indent,' ');
                        ...
                        break;
	case ID:	        printf("%*c|--VarDecl %s %s\n",indent,' ',T->type_id, T->type==INT?"int":"float");
                        break;
  ...
  case MOD:
	...
	case UMINUS:    printf("%*c%s\n",indent,' ',T->type_id);
                    display(T->ptr[0],indent+3);
                    break;
    case FUNC_CALL: printf("%*c|--UnaryExp %s\n",indent,' ',T->type_id);
                    display(T->ptr[0],indent+3);
                    break;
	case ARGS:      i=1;
